...
function moveAndDeleteFiles(inWatchMode) {
  ...
  return {
    ...
    closeBundle() {
      ...
      const source = path.join(__dirname, 'dist/css/errors.css');
      const destination = path.join(__dirname, 'errors/css/main.css');
      const destDir = path.join(__dirname, 'errors/css/')
      ...
      const viteDirToDelete = path.join(__dirname, 'dist/vite-src');
...
const INTERMEDIARY_ASSETS = ['assets/scss/_sprite.scss', 'assets/images/sprite.png', 'assets/images/sprite@2x.png'].map(p => '**/' + p);
